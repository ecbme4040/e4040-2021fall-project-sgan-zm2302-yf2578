...
# the many-to-many lstm reference: https://wandb.ai/ayush-thakur/dl-question-bank/reports/One-to-Many-Many-to-One-and-Many-to-Many-LSTM-Examples-in-Keras--VmlldzoyMDIzOTM
    ...
    def call(self, inputs, n_repeat=1, return_mean=True, return_var=False):
        ...
        if return_mean:
            if return_var is True:
                # var
                out_mean = tf.reshape(out_mean, n_repeat, axis=0)
                out_var = tf.reshape(tf.square(out - out_mean), [-1, self.num_f_repeat])
                out_var = tf.reduce_mean(out_var, axis=1, keepdims=True)
